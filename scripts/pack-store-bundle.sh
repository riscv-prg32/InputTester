...
cp "$repo_dir/dist/inputtester_esp32c6.prg32" "$stage_dir/inputtester_esp32c6.prg32"
cp "$repo_dir/dist/inputtester_qemu.prg32" "$stage_dir/inputtester_qemu.prg32"

(cd "$prg32_repo" && python3 -m prg32 store pack-bundle \
  --manifest "$stage_dir/manifest.json" \
  --out "$repo_dir/dist/inputtester-store-bundle.zip")

echo "$repo_dir/dist/inputtester-store-bundle.zip"
